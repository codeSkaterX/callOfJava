...
public class Unit {
	...
	public Unit(TiledMap map, int collisionLayer, String title, int life, int attack, int defense, int movement, int x, int y) {
		this.map = map;
		xPos  = x*tileSize;
		yPos  = y*tileSize;
	...
	public int moveUp(int collisionLayer){
		if(movement>=1){
	...
	public int moveDown(int collisionLayer){
		if(movement>=1){
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("Units"), emptyID);
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("collision"), tempID);
			yPos += tileSize;
	...
	public int moveLeft(int collisionLayer){
		if(movement>=1){
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("Units"), emptyID);
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("collision"), tempID);
			xPos -= tileSize;
	...
	public int moveRight(int collisionLayer){
		if(movement>=1){
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("Units"), emptyID);
			map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("collision"), tempID);
			xPos += tileSize;
	...
	public boolean isAlive(){
		if(hp > 0)
			return true;
		map.setTileId(Math.round(xPos/32), Math.round(yPos/32), map.getLayerIndex("Units"), grave);
		return false;
	}
